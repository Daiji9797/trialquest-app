...
// Anonymous IDs generated by the frontend are always "user-" + 8 random base36 chars.
// GitHub userIds are numeric strings. Unauthenticated requests must not access non-anonymous IDs.
...
function checkAuthorized(principal, userId) {
  if (principal?.userId) {
    // Authenticated: must match their own GitHub userId
    return principal.userId === userId;
  }
  // Unauthenticated: may only access anonymous-style IDs
  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (request.method === 'POST' || request.method === 'PATCH') {
        ...
        let resource = container ? await readProfile(container, userId) : memoryProfiles.get(userId) || null;
        if (!resource) {
          // プロファイルが存在しない場合は最小構成で作成する
          resource = {
            id: userId,
            userId,
            skills: { health: 1, knowledge: 1, relationship: 1, action: 1, creation: 1 },
        ...
        if (body.suggestions !== undefined) {
          resource.suggestions = body.suggestions;
        }
